...
public class PhieuGiamGiaRepository {
    ...
    public boolean add(PhieuGiamGia1 phieu) {
        int check = 0;
        String sql = "INSERT INTO Voucher (ngay_bat_dau, ngay_het_han, ten_giam_gia, phan_tram_giam_gia, gia_tri_don_hang_toi_thieu, so_lan_su_dung_toi_da, trang_thai, mo_ta) "
                + "VALUES (?,?,?,?,?,?,?,?);";
        try (Connection con = DBConnect_An.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
